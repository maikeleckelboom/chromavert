...
import { useSteps } from '~/modules/slider/runtime/composables/useSteps'
...
function handleSwipe(_event: PointerEvent) {
  ...
  if (isNumber(modelValue.value)) {
    if (isDefined(props.step) && props.step !== 'any') {
      modelValue.value = getClosestStep(currentValue)
      return
    }
