...
<template>
  <NuxtLink ref="itemRef" v-slot="{ active }" :to="item.path" class="v-nav-item">
    <span class="v-state-indicator" />
    <Transition>
      <Icon v-if="active && activeIcon" :name="activeIcon" class="v-icon" />
      <Icon v-else-if="baseIcon" :name="baseIcon" class="v-icon" />
    </Transition>
